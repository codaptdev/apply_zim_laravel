    ...
    <div class="my-10 w-full md:px-20 p-2 grid grid-cols-1 gap-3 ">
        ...
        @forelse ($schools as $school)
            <div class="w-full rounded-xl p-5 mx-2 bg-stone-50  flex flex-row items-center justify-start border-2 border-slate-300">
                {{-- Logo --}}
                <div class="flex border-slate-200 w-20  h-20 mb-5 md:mb-0 bg-slate-300 mr-5 rounded-xl" >
                ...
                <div class="flex flex-col h-full">
                    ...
                    <div class="md:flex grid grid-cols-2 gap-2 md:flex-row w-full md:mt-0 mt-2">
                        ...
                        <a  href="?key=town_city&value={{$school->town_city}}" class="bg-green-200 font-semibold text-green-800  py-1 px-4 mr-2 md:my-4 rounded-full hover:bg-green-400 cursor-pointer">{{strtoupper($school->town_city)}}</a>
                        <a  href="?key=level&value={{$school->level}}" class="bg-amber-200 font-semibold text-amber-800  py-1 px-4 mr-2 md:my-4 rounded-full hover:bg-amber-400 cursor-pointer">{{strtoupper($school->level)}}</a>
                    </div>
                </div>
            </div>
        @empty
